    ...
    async submitTxn(
        ...
        try {
            ...
            if (txnType == "invokeTxn") {
              ...
              else if (txnType == "invokeListEnergy") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
            else if (txnType == "invokePurchaseEnergy") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
            else if (txnType == "invokeAuditSale") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
            else if (txnType == "queryEnergyDetails") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            }
            else if (txnType == "invokeExcessEnergy") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
            else if (txnType == "invokeLoan") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
